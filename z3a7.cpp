...
main (int argc, char *argv[])
{
	...
	try
	{
	    ...
	    if (argc != 4)
	    {
	        ...
	        //return -1;
	        throw std::ios::failure("nem megfelelő számú argumentum");
	    }
	    ...
	    if (*((*++argv) + 1) != 'o')
	    {
	        //usage ();
	        //return -2;
	        throw std::ios::failure("nincs -o kapcsoló");
	    }
	    ...
	    if (!beFile)
	    {
	        ...
	        //usage ();
	        //return -3;
	        throw std::ios::failure("hibásan megadott bemeneti fájl");
	    }
	...
	{
		std::cout<<"Hiba: "<<message.what()<<endl;	
		return 1;
	}
}
